...
from models.s2v_gammaTransformer.models.sentence_encoder_model import SentenceEncoderModel
from models.s2v_gammaTransformer.models.nli_matching_model import NliClassifierModel
from models.s2v_gammaTransformer.sentence_processing import get_batch
from models.s2v_gammaTransformer.configs import configs
...
model = tf.keras.models.Model(inputs=[input_sentence1, input_sentence1_mask,
                                      ...
                                      input_sentence2_transformer_mask],
                              outputs=[sent1_s2v, nli_predictions])
model.load_weights("models/s2v_gammaTransformer/save/"+config['name']+"/model")


def generate_s2v(sentence_emb):
    x, _ = get_batch(sentence_emb, config)
    s2v, prediction = model.predict(x, batch_size=config['batch_size'])
    return (s2v, prediction)
